...
B<qc--> [I<option> | I<file>] ...  E<10>
B<qc--.opt> [I<option> | I<file>] ... 
...
The binary called B<qc--> is a bytecode file; 
B<qc--.opt>, if availabe, is a native-code binary.
Use B<qc--.opt> if you have it; the speedup is impressive.
...
Usually B<qc--> generates machine code but it can also generate code for
the Quick C-- interpreter qc--interp(1); see the B<-interp> option
below. 
...
Interpreter source: link. A C<file.qs> is source code understood by the
...
If invoked with no arguments, B<qc--> summarizes its options on the
standard error and exits with exit code C<1>.
...
into the I<first> C-- file compiled on this run.
Every C-- program must contain exactly one instance of this digital signature.
Therefore, the B<-globals> option should be used on exactly one of the
compilation runs used to build the program.

=item B<-stop> B<.>I<extension>

When the compiler derives a file whose name ends in B<.>I<extension>,
stop translating and keep the file.
For example, the B<-c> and B<-S> options used by some C compilers can be
rendered as B<-stop .o> and B<-stop .s> respectively.

=item B<-keep> B<.>I<extension>

Keep all derived files whose names end in B<.>I<extension>.
...
Emit code for the Quick C-- interpreter qc--interp(1). When this option
is used, the options B<-c> and B<-S> have the same effect because
interpreter code makes no distinction between assembly code and object
code.  The only intermediate file format I<file>C<.qs>.
...
Output this manual page to stdout stop with exit code C<0>. This option
...
# =item B<-e> I<chunk>
#
# Evaluate the Lua code I<chunk> in B<qc-->'s Lua interpreter. 
#
=back
