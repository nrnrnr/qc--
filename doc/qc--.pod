...
an executable file (by default named C<a.out>). An option I<option> has
the form C<->I<name> or I<name>C<=>I<value>, hence any argument with an
equal sign is treated as an option.  Any other argument B<qc--> regards
as a I<file> name.  

The B<qc--> compiler is controlled by a built-in Lua interpreter that
executes a compiled-in Lua program. By modifying the state of the
