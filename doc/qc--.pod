...
arguments and compiles, assembles, and links the files on the command
line into an executable (by default C<a.out>). An option I<option> has
the form C<->I<name> or C<->I<name>C<=>I<value>, any other argument
B<qc--> regards as a I<file> name.  The treatment of a file depends on
the ending of its name:
...
file is compiled to assembly code and assembled to an object file that
is linked with the object files derived from the other I<file>
arguments.
...
I<What do we say about one string versus mutiple strings?>
...
Like B<-as.options>, but for the linker. Instead of using options B<-L>
and B<-l> one could use B<-as.options> to pass these options to the
linker.  However, the specific options B<-L> and B<-l> have
traditionally been accepted by compilers and thus we provide them
despite have the more general B<-as.options>.  
