...
=item B<Interp.path=>I<path>

Define an alternate path to the interpreter qc--interp(1). The
interpreter is called for linking object files for the interpreter.

=item B<Interp.options>=I<options>

Define options passed to the interpreter qc--interp(1) when called for
linking.
