...
group,'' to consist of \emph{Christian, Glenn, John, Kevin, and Norman.}
\item
...
The problem is to identify techniques that lead to solid code, which
...
such a cell. In principle, we need only one such cell.  (Or one per
thread, if scheduling is pre-emptive.)
We should probably allocate one per activation, in the stack slot of
the current frame.
\item
