...
static	int	nofltconv(va_list*, Fconv*);
...
numbconv(va_list *arg, Fconv *fp)
{
	...
	case FVLONG|FLONG:
                LONGLONGSTMT(vl = va_arg(*arg, vlong);)
		break;

	case FUNSIGN|FVLONG|FLONG:
		LONGLONGSTMT(vl = va_arg(*arg, uvlong);)
		break;
	...
	if(fp->f3 & FVLONG) {
		LONGLONGSTMT(if(!(fp->f3 & FUNSIGN) && vl < 0) {
			vl = -vl;
			f = 1;
		})
	} else {
...
static int
nofltconv(va_list *arg, Fconv *f)
{
	double d;

	d = va_arg(*arg, double);	/* need to advance the stack correctly */
	USED(d);
	return noconv(arg, f);
}
